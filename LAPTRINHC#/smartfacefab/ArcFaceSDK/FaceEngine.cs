...
namespace ArcFaceSDK
{
    ...
    {
        /// <summary>
        /// Danh sách engine xử lý khuôn mặt
        /// </summary>
        public List<EngineModel> lsEngine = new List<EngineModel>();       

        private int totalEngine = 1;
        
        public int getTotalEngine()
        {
            return totalEngine;
        }

        public void setTotalEngine(int totalEngine)
        {
            this.totalEngine = totalEngine;
        }
        ...
        public int ASFInitEngine(DetectionMode detectMode, ASF_OrientPriority detectFaceOrientPriority, int detectFaceMaxNum, int combinedMask)
        {
            ...
            for (int i = 0; i < getTotalEngine(); i++)
            {
                IntPtr pEngine = IntPtr.Zero;
                int retCode = ASFFunctions.ASFInitEngine(detectMode, detectFaceOrientPriority, detectFaceMaxNum, combinedMask, ref pEngine);
                if (retCode == 0)
                {
                    EngineModel engineModel = new EngineModel(pEngine, true);
                    lsEngine.Add(engineModel);
                    Console.WriteLine("ENGINE: " + engineModel.PtrEngine);
                }
            }
            Console.WriteLine("TOTAL ENGINE INIT: " + lsEngine.Count);
        ...
        {
            return 0;// ASFFunctions.ASFSetFaceShelterParam(pEngine1,shelterThreshhold);
        }


        public MultiFaceInfo ASFDetectFaces(EngineModel engineModel, Image image, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, ASF_DetectModel detectModel = ASF_DetectModel.ASF_DETECT_MODEL_RGB, bool isEx = false)
        {
            ...
            try
            {
                ...
                IntPtr pEngine = engineModel.PtrEngine;
            ...
            {
                //ReleaseEngine(engineModel);
            }

            return multiFaceInfo;
        }

        public MultiFaceInfo ASFDetectFaces(Image image, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, ASF_DetectModel detectModel = ASF_DetectModel.ASF_DETECT_MODEL_RGB, bool isEx = false)
        {
        ...
        public void releaseEngine(EngineModel engineModel)
        {
            lock (lsEngine)
            {
        ...
        public EngineModel getEngineProcess()
        {
            lock (this)
            {
                ...
                {
                    engineProcess = lsEngine.Find(e => e.EngineFree == true);
                    if (engineProcess != null)
                    {
                        engineProcess.EngineFree = false;
                        break;
                    }
                    //Thread.Sleep(20);
                }
        ...
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFUpdateFaceData(EngineModel engineModel, Image image, ref MultiFaceInfo multiFaceInfo, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            int retCode = -1;
        ...
        /// <param name="imageFormat">图片格式</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFUpdateFaceDataEx(EngineModel engineModel, Image image, ref MultiFaceInfo multiFaceInfo, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8)
        {
            return ASFUpdateFaceData(engineModel, image, ref multiFaceInfo, imageFormat, true);
        }
        ...
        /// <param name="isEx">是否调用Ex类接口</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFImageQualityDetect(EngineModel engineModel, Image image, MultiFaceInfo multiFaceInfo, out float confidenceLevel, int faceIndex = 0, bool isMask = false, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, ASF_DetectModel detectModel = ASF_DetectModel.ASF_DETECT_MODEL_RGB, bool isEx = false)
        {
            int retCode = -1;
            confidenceLevel = 0f;
        ...
        /// <param name="detectModel">预留字段，当前版本使用默认参数即可</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFImageQualityDetectEx(EngineModel workEngine, Image image, MultiFaceInfo multiFaceInfo, out float confidenceLevel, int faceIndex = 0, bool isMask = false, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, ASF_DetectModel detectModel = ASF_DetectModel.ASF_DETECT_MODEL_RGB)
        {
            return ASFImageQualityDetect(workEngine, image, multiFaceInfo, out confidenceLevel, faceIndex, isMask, imageFormat, detectModel, true);
        }
        ...
        /// <param name="isEx">是否调用Ex类接口</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFProcess(EngineModel workEngine, Image image, MultiFaceInfo multiFaceInfo, int combinedMask, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            int retCode = -1;
            ...
            try
            {
                ...
                IntPtr pEngine = workEngine.PtrEngine;
                MemoryUtil.StructureToPtr(multiFaceInfoStruct, pMultiFaceInfo);
                //调用SDK接口
                if (isEx)
                {
        ...
        /// <param name="imageFormat">图片格式</param>
        /// <param name="workEngine">图片格式</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFProcessEx(EngineModel workEngine, Image image, MultiFaceInfo multiFaceInfo, int combinedMask, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8)
        {
            return ASFProcess(workEngine, image, multiFaceInfo, combinedMask, imageFormat, true);
        }
        ...
        /// <param name="isEx">是否调用Ex类接口</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFProcess_IR(EngineModel engineModel, Image image, MultiFaceInfo multiFaceInfo, int combinedMask, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_GRAY, bool isEx = false)
        {
            int retCode = -1;
        ...
        /// <param name="imageFormat">图像格式</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFProcessEx_IR(EngineModel engineModel, Image image, MultiFaceInfo multiFaceInfo, int combinedMask, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_GRAY)
        {
            return ASFProcess_IR(engineModel, image, multiFaceInfo, combinedMask, imageFormat, true);
        }
        ...
        /// <param name="isEx">是否调用Ex类接口</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFFaceFeatureExtract(EngineModel engineModel, Image image, MultiFaceInfo multiFaceInfo, ASF_RegisterOrNot registerOrNot, out FaceFeature faceFeature, int faceIndex = 0, bool isMask = false, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            int retCode = -1;
        ...
        /// <param name="imageFormat">图片格式</param>
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFFaceFeatureExtractEx(EngineModel engineModel, Image image, MultiFaceInfo multiFaceInfo, ASF_RegisterOrNot registerOrNot, out FaceFeature faceFeature, int faceIndex = 0, bool isMask = false, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8)
        {
            return ASFFaceFeatureExtract(engineModel, image, multiFaceInfo, registerOrNot, out faceFeature, faceIndex, isMask, imageFormat, false);
        }
        ...
        /// <returns>返回0表示正常；返回负数请根据ErrorCodeUtil类注释查看；其他值请在官网-帮助中心查询</returns>
        public int ASFFaceFeatureCompare(EngineModel engineModel, FaceFeature faceFeature1, FaceFeature faceFeature2, out float similarity, ASF_CompareModel compareModel = ASF_CompareModel.ASF_ID_PHOTO)
        {
            int retCode = -1;
            similarity = 0f;
        ...
        public int ASFGetAge(EngineModel workEngine, out AgeInfo ageInfo)
        {
            int retCode = -1;
            ageInfo = new AgeInfo();
        ...
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFGetGender(EngineModel workEngine, out GenderInfo genderInfo)
        {
        ...
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFGetFace3DAngle(EngineModel engineModel, out Face3DAngle faceAngle)
        {
        ...
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFSetLivenessParam(EngineModel engineModel, float rgbThreshold = 0.5f, float irThreshole = 0.7f)
        {
            int retCode = -1;
        ...
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFGetLivenessScore(EngineModel engineModel, out LivenessInfo livenessInfo)
        {
        ...
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFGetLivenessScore_IR(EngineModel engineModel, out LivenessInfo livenessInfo)
        {
            livenessInfo = new LivenessInfo();
            int retCode = -1;
        ...
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFGetMask(EngineModel engineModel, out MaskInfo maskInfo)
        {
        ...
        /// <returns>返回0表示正常；返回其他值请在官网-帮助中心查询</returns>
        public int ASFGetFaceLandMark(EngineModel engineModel, out LandMarkInfo faceLandMark)
        {
        ...
        {
            for (int i = 0; i < lsEngine.Count; i++)
            {
                var pEngine = lsEngine[0].PtrEngine;

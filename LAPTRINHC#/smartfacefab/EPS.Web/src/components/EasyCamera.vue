<template>
    <fullscreen-view-camera @approve="(picture) => { this.$emit('approve', picture); this.$emit('input', picture); }"
    ...
    <standard-view-camera @loading="(loading) => {this.$emit('loading', true)}"
                          :overlay-mask="overlayMask"
                          :start-on-mounted="startOnMounted"
                          :visible-overlay="visibleOverlay"
...
<script>
    ...
    import StandardViewCamera from 'easy-vue-camera/src/components/StandardViewCamera.vue';
    import OutputType from 'easy-vue-camera/src/OutputType';
    import DeviceUtils from 'easy-vue-camera/src/utils/DeviceUtils';

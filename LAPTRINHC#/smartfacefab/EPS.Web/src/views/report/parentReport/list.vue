    ...
    <div class="animated fadeIn">
        <b-row>
            <b-col lg="12">
                <b-card>
                    <div slot="header">
                        <strong> {{ $t('Reporting.ParentReport.List.Header') }} </strong>
                    </div>
                    <b-form @submit.prevent="search" class="form-search">
                        <b-row style="margin-left: 10px;">
                            ...
                            <b-col md="4">
                                <b-form-group :label="this.$t('School.Parent.Detail.Label.NameStudent')"
                                              label-for="name"
                                              :label-cols="4"
                                              :horizontal="true"
                                              label-align-md="left">
                    ...
                    <b-grid :searchForm="searchForm"
                            ...
                            dataUrl="event/parent"
                            gridName="Reporting.ParentReport.List.Table.TableName"
                            ref="eventTable">
                                ...
                                <b-row>
                                    <b-col md="6">
                                        <b-form-group :label="this.$t('Guess.Detail.Form.Phone')"
                                                      ...
                                                      :horizontal="true"
                                                      label-align-md="left">
                                            <label class="col-form-label"> {{ event.phoneNumber }} </label>
                                        </b-form-group>
                                    </b-col>
                                    <b-col md="6">
                                        <b-form-group label="Email:"
                                                      ...
                                                      :horizontal="true"
                                                      label-align-md="left">
                                            <label class="col-form-label"> {{ event.email }} </label>
                                        </b-form-group>
                                    </b-col>
                                </b-row>
                                <b-row>
                                ...
                                <b-table striped hover :items="lststudent" :fields="studentFields" show-empty>
                                    <template #empty="scope">
                                        <div class="invalid-feedback"
                                            ...
                                            {{ $t('School.Parent.Detail.Label.Student') }}
                                        </div>
                                    </template>
                                    ...
                                    <template v-slot:cell(personCode)="row">
                                        <router-link :to="{ path: '/school/student/detail/' + row.item.personId}"
                                                     target="_blank">
                                            {{row.item.personCode}}
                                        </router-link>
                                    </template>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                export_fields: {
                    'Reporting.ParentReport.List.Table.PersonCode': 'personCode',
                    'School.Parent.Detail.Label.Namestudent': 'person',
                    'School.Parent.List.Table.NumberPhone': 'phoneNumber',
                    'Email': 'email',
                    'Reporting.Event.List.Table.Machine': 'machine',
                ...
                gridOptions: {
                    fields: [
                        { key: 'personCode', label: this.$t('Reporting.ParentReport.List.Table.PersonCode'), sortable: false },
                        { key: 'person', label: this.$t('School.Parent.Detail.Label.Namestudent'), sortable: false },
                        { key: 'phoneNumber', label: this.$t('School.Parent.List.Table.NumberPhone'), sortable: false },
                        { key: 'email', label: 'Email', sortable: false },
                        { key: 'machine', label: this.$t('Reporting.Event.List.Table.Machine'), sortable: false, },
                        { key: 'accessDateDayFormat', label: this.$t('Reporting.Event.List.Table.Day'), sortable: false },
                        { key: 'accessDateTimeFormat', label: this.$t('Reporting.Event.List.Table.Hours'), sortable: false, },
                        { key: 'wearMaskName', label: this.$t('Reporting.Event.List.Table.FaceMask'), sortable: false, },
                        { key: 'temperatureFormat', label: this.$t('Reporting.Event.List.Table.Temp'), sortable: false, },
                ...
                lstperson: [],
                lstHD: [
                    { id: '1001', text: this.$t('Reporting.Event.List.Table.ListHD1') },
                    { id: '1002', text: this.$t('Reporting.Event.List.Table.ListHD2') },
                    { id: '1003', text: this.$t('Reporting.Event.List.Table.ListHD3') },
                    { id: '1004', text: this.$t('Reporting.Event.List.Table.ListHD4') },
                    { id: '3001', text: this.$t('Reporting.Event.List.Table.ListHD5') },
                    { id: '0002', text: this.$t('Reporting.Event.List.Table.ListHD6') },
                ],
                lstStatus: [
                    { id: '1', text: this.$t('Reporting.Event.List.Table.Status0') },
                    { id: '0', text: this.$t('Reporting.Event.List.Table.Status1') }
                ],
                isAdmin: false,
                darkModal: false,
                event: {
                ...
                searchTemp: true,
                studentFields: [
                    { key: 'personCode', label: this.$t('School.Class.List.Table.Code'), sortable: false },
                    { key: 'fullName', label: this.$t('School.Parent.Detail.Label.Namestudent'), sortable: false, },
                    { key: 'gradeName', label: 'Khối', sortable: false },
        ...
        methods: {
            ...
            async loadEventDetail(eventId) {
                ...
                return this.$services.get(`event/${eventId}`).done(event => {
                    vm.event = event;
                    if (event.personId) {
                        this.loadListStudentByParentId(event.personId);
                    }
                });
            },
            async loadListStudentByParentId(id) {
                var vm = this;
                return this.$services.get(`person/parent/getliststudents/${id}`).done(res => {
                    vm.lststudent = res;
                });
            },
            refresh() {

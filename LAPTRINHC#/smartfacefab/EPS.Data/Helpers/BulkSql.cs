...
namespace EPS.Data.Helpers
{
    public class BulkSql
    {
        ...
        public void InsertData<T>(List<T> list, string tableName)
        {
            ...
            {
                conn.Open();
                using (SqlTransaction sqlTransaction = conn.BeginTransaction(IsolationLevel.ReadCommitted))
                {
...
";
        ...
        private DataTable ToDataTable<T>(IList<T> data)
        {
            ...
            for (int i = 0; i < properties.Count; i++)
            {
                ...
                if (propertyType.IsPrimitive || propertyType == typeof(bool) || propertyType == typeof(Boolean) || propertyType == typeof(DateTime) || propertyType == typeof(string) || propertyType == typeof(decimal) || propertyType == typeof(Guid))
                {

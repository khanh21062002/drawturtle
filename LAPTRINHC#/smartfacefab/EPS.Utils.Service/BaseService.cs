...
namespace EPS.Utils.Service
{
    public class BaseService : IBaseService
    {
        ...
        public virtual bool Any<TEntity, TDto>(params Expression<Func<TDto, bool>>[] predicates)
            where TEntity : class
        {
            return _repository.All<TEntity>().ProjectTo<TDto>(_mapper.ConfigurationProvider).WhereMany(predicates).Any();
        }

        public virtual bool Any<TEntity, TDto>(Expression<Func<TEntity, TDto>> mapping, params Expression<Func<TDto, bool>>[] predicates)
            where TEntity : class
        {
            return _repository.All<TEntity>().Select(mapping).WhereMany(predicates).Any();
        }

        public virtual async Task<bool> AnyAsync<TEntity, TDto>(params Expression<Func<TDto, bool>>[] predicates)
            where TEntity : class
        {
            return await _repository.All<TEntity>().ProjectTo<TDto>(_mapper.ConfigurationProvider).WhereMany(predicates).AnyAsync();
        }

        public virtual async Task<bool> AnyAsync<TEntity, TDto>(Expression<Func<TEntity, TDto>> mapping, params Expression<Func<TDto, bool>>[] predicates)
            where TEntity : class
        {

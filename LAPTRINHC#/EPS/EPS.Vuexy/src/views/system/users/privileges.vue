        ...
        <b-card>
          <div slot="header">
            <strong class="card-title">{{
              $t("System.User.Privileges.Header")
            }}</strong>
          </div>
          <b-form @submit.prevent="save" style="min-height: 500px">
            <b-row>
              <b-col md="4">
                <b-form-group
                  :label="
                    this.$t('System.User.Privileges.Label.CategoriesList')
                  "
                  label-for="module"
                >
                  <treeselect
...
export default {
  ...
  created() {
    var vm = this;
    //console.log(this.authorize(['ViewRole']));
    this.$services
  ...
  methods: {
    ...
    save() {
      ...
      this.$services
        .put(`users/${this.$route.params.userId}/privileges?privileges=${data}`)
        .done(() => {

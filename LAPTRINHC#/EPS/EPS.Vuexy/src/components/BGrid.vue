<template>
  <div style="width: 100%">
    <b-row>
      <b-col md="7">
        <h4 v-if="gridNamei18n != ''">{{ gridNamei18n }}</h4>
    ...
    <b-row v-if="options.totalRows > 0 && footerable">
      <b-form class="col-md-12 page">
        <b-col md="6">
          {{ $t("Bgrid.Sentence.ShowResultFrom") }} <b>{{ firstItem }}</b>
          {{ $t("Bgrid.Sentence.To") }} <b>{{ lastItem }}</b>
          {{ $t("Bgrid.Sentence.Intotal") }} <b>{{ options.totalRows }}</b>
          {{ $t("Bgrid.Sentence.Record") }}
        </b-col>
        <b-col md="6" class="my-1 pull-right input-group">
          <label class="control-label ml-auto">
            {{ $t("Bgrid.Pagesize") }}
          </label>
          <b-form-select
            ...
            class="select-grid"
            style="flex: none; margin: 0px 5px; padding: 6px 28px 6px 12px"
          ></b-form-select>
          <b-pagination
            align="right"
          ...
          <b-form-select
            v-model="options.page"
            :options="lstPageNum"
            style="flex: none; margin-left: 5px; padding: 6px 28px 6px 12px"
          ></b-form-select>
          <b-button type="button" class="btn-grid" @click="mdConfig = true">
            <i class="fa fa-cog"></i>
          </b-button>
        </b-col>
        <!-- <ul class="pagination">
    ...
    >
      <div>
        <div class="form-row form-group">
          <div class="col-md-6 col-switch" v-for="(item, index) in lstColumns">
            <label class="col-form-label col-sm-10"> {{ item.text }} </label>
            <div class="col-sm-2">
              <label
                class="mr-1 c-switch form-check-label c-switch-pill c-switch-info c-switch-label"
              >
                <input
                  type="checkbox"
      ...
      <template #footer class="footer-model">
        <b-button type="button" variant="primary" @click="saveConfig"
          ><i class="fa fa-floppy-o"></i>{{ $t("Button.Submit") }}</b-button
        >
        <b-button
          type="button"
          variant="secondary"
          @click="mdConfig = false"
          style="width: 120px"
  ...
  computed: {
    ...
    lstPageNum() {
      ...
      let totalPage = Math.max(
        1,
        Math.floor(this.options.totalRows / this.options.itemsPerPage)
      );
  ...
  methods: {
    ...
    getItems(ctx) {
      ...
      return this.$services
        .get(window.encodeURI(this.dataUrl), formData)
        .then(
          (response) => {

...
var services = {
  ...
  TOKEN_END_POINT: isDevEnv
    ? "http://localhost:1938/api/token/auth"
  ...
  getResourceUrl: function (path) {
    if (path) {
      return (
        this.SERVICE_END_POINT +
        "/" +
        this.STOREDCALLS_FOLDER +
        "/" +
        path.substring(this.EMAS_STOREDCALLS_FOLDER.length)
      );
    } else {
      return "";
    }
  ...
  getUploadResourceUrl: function (path) {
    if (path) {
      return (
        this.SERVICE_END_POINT +
        "/" +
        this.UPLOADEDCALLS_FOLDER +
        "/" +
        path.substring(this.EMAS_UPLOADEDCALLS_FOLDER.length)
      );
    } else {
      return "";
    }
